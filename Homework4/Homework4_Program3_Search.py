...
s = set(map(str, input('Enter search words with Operator AND or OR: ').split()))

print("You have entered following search:")
print(s)

if ('OR' in s) and ('AND' in s):
    OP="AND"
    
elif ('AND' in s) and ('OR' not in s):
    OP="AND"
    
elif ('OR' in s) and ('AND' not in s):
    OP="OR"
...
print("Performing " + OP + " search for: ")
print(s)

for i,quote in enumerate(data_list):
    found_at1 = quote.find(query1)
    found_at2 = quote.find(query2)
    if OP=="OR":
        if ( found_at1 >= 0):
